...
| `fleet/` | Batch migration across the whole app estate: `fleet.R <verb>` (`import-unbuilt`, `process`, `report`, `rollup`, `status`), the `manifest.csv` ledger, the `master*.csv` record, `flags.csv`, `qlik_api.R` (DESIGN §8.6) |
