...
#   4. enforce_reserved_word_case - Qlik keywords and functions -> UPPER
#
# Casing runs last on purpose: earlier passes splice in new tokens (the "AS"
# of an added alias, a relocated comma), so finishing with the casing pass
# guarantees the whole output is consistently cased no matter what they
# emitted. It is otherwise order-independent - it only rewrites the text of
# WORD tokens and never changes structure.
#
...
warnings_all <- c(r1$warnings, r2$warnings, r3$warnings, r4$warnings)
...
writeLines(detokenize(r4$tokens), output_path)
...
#   r4$changes   (words recased; $kind is "keyword" or "function")
